...
func TestTheExemptionListDoesNotGrowQuietly(t *testing.T) {
	want := []string{
		...
		"auth.register_google -> users.deleted_at,email,failed,id,locked_until,pass_hash,token_version",
		...
		"auth.user_by_google -> users.deleted_at,email,failed,id,locked_until,pass_hash,token_version",
