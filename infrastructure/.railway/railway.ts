...
export default defineRailway((ctx) => {
  // DO NOT use ctx.isEnvironment here. The SDK compares with `===`
  // (node_modules/railway/dist/iac/index.js:85), so it is exact and
  // case-sensitive. The two environments were renamed on 2026-09-10 from
  // DEV/PROD to `Devevelopment`/`Production` -- same IDs, new names -- and after
  // that `isEnvironment("PROD")` and `isEnvironment("production")` were BOTH
  // false against `Production`. `prod` silently became false, which would have
  // configured the production environment as DEV: tracking the `dev` branch,
  // writing into the `dev-files` bucket, announcing APP_ENV=DEV, and pointing
  // PUBLIC_ORIGIN and WEBHOOK_PUBLIC_ORIGIN at DEV so Mercado Pago would deliver
  // webhooks to the wrong origin.
  //
  // Matching a prefix survives a rename. Refusing to guess is the other half:
  // an unrecognised name throws instead of falling through to DEV, because that
  // fall-through is exactly the failure this comment exists about.
  const envName = String((ctx as { environment?: string; environmentName?: string }).environment
    ?? (ctx as { environmentName?: string }).environmentName
    ?? "");
  const prod = /^prod/i.test(envName);
  const isDev = /^dev/i.test(envName);
  if (!prod && !isDev) {
    throw new Error(
      `railway.ts: unrecognised environment ${JSON.stringify(envName)}. `
      + `Expected a name starting with "dev" or "prod". Refusing to guess, `
      + `because guessing means DEV settings applied to production.`);
  }
  ...
  const repo = github("alpadev-fx/ai-from-scratch", { branch: prod ? "main" : "dev" });
  ...
  const api = service("api", {
    ...
    env: {
      ...
      MAIL_FROM: preserve,
      // Railway Bucket credentials, S3-compatible. `bucket()` returns a
      // BucketNode with only `address`, `type` and `config` -- there is no
      // `.env` surface the way `postgres()` has one -- so these cannot be
      // references and have to be preserved copies.
      //
      // They are declared because they already EXIST on the production api
      // (AWS_S3_BUCKET_NAME = prod-files-…, endpoint https://t3.storageapi.dev)
      // and IaC deletes every variable this file does not declare. Undeclared,
      // the next apply would strip the production api of its storage
      // credentials and every PDF, certificate and upload would stop being
      // written, with nothing in the plan calling that out beyond six
      // `Delete variable` lines. The same mistake already removed two of the
      // four AI providers before they were declared.
      //
      // The DEV api has none of these set: it cannot write to `dev-files` at
      // all today. Fill them from the DEV bucket's own credentials; do NOT copy
      // the production values across, which is the whole point of two buckets.
  ...
  const web = service("web", {
    ...
    healthcheck: "/healthz",
    // Railway IaC cannot register custom domains. aifromscratch.shop is added
    // out of band with `railway domain` after the PROD service exists.
    env: {
  ...
  // The other four defence agents. `security verify` inside the image reports
  // "5 agents, exactly one of which may act", and the one is neo.
  //
  // They share oracle's image and differ only in the binary they start, because
  // security/Dockerfile ships all six binaries in one FROM scratch layer and each
  // one binds its own durable queue from the table in security/binding/binding.go.
  //
  // READ THIS BEFORE PAYING FOR morpheus, trinity AND smith. Their queues are fed
  // by `defense.perimeter.request`, `defense.host.request` and
  // `defense.scan.request`, and NOTHING in the repository publishes any of the
  // three -- the same shape of hole as `league.week.close`, which no scheduler
  // ever emitted either. Declared as they are, the three consume from queues
  // nobody fills and idle forever at the cost of a container each. They need a
  // publisher (a cron, or an operator command) before they do any work.
  //
  // morpheus and trinity have a second problem: they were written for the
  // Raspberry Pi. Their own headers say they look at "sshd, the listening sockets
  // and the firewall ruleset" and that "every exposure Morpheus can find is a LAN
  // exposure". A Railway container has no sshd, no firewall ruleset to read and no
  // LAN, so on this platform they would report an empty perimeter even with a
  // publisher. smith is the one that still means something here: it asks what a
  // foothold inside THIS container gets, which is per-container by nature.
  //
  // No volumes for these three. DEFENSE_AUDIT appears only in the Dockerfile, in
  // cmd/security/main.go and in internal/config/config.go, and only
  // `security audit verify` errors on an empty path -- the reporters do not need a
  // writable one to boot, so a volume each would be three volumes for nothing.
  const agentBuild = {
    builder: "DOCKERFILE" as const,
    dockerfilePath: "security/Dockerfile",
    watchPatterns: ["security/**", "queue/**"],
  };
  const agentEnv = {
    AMQP_URL: broker.env.AMQP_URL,
    APP_ENV: prod ? "PROD" : "DEV",
  ...
  const morpheus = service("morpheus", {
    ...common, root: "app_ai_from_scratch", build: agentBuild,
    start: "/morpheus",
    deploy: { restartPolicyType: "ON_FAILURE" },
    env: agentEnv,
  });
  const trinity = service("trinity", {
    ...common, root: "app_ai_from_scratch", build: agentBuild,
    start: "/trinity",
    deploy: { restartPolicyType: "ON_FAILURE" },
    env: agentEnv,
  });
  const smith = service("smith", {
    ...common, root: "app_ai_from_scratch", build: agentBuild,
    start: "/smith",
    deploy: { restartPolicyType: "ON_FAILURE" },
    env: agentEnv,
  });
  // neo is the ONLY agent that may change the running system, and its own header
  // states the threat model: AMQP has no per-message authentication, so anyone who
  // can publish to the exchange can command neo. DEFENSE_MODE stays `propose` --
  // it decides and records, it does not apply -- and it keeps a volume because the
  // audit trail of what it decided is the only record that exists. Do not move it
  // off `propose` without deciding who is allowed to publish to the broker.
  const neoState = volume("defense-neo", { sizeMB: 512, region: "iad" });
  const neo = service("neo", {
    ...common, root: "app_ai_from_scratch", build: agentBuild,
    start: "/neo",
    volumeMounts: { "/var/lib/defense": neoState },
    deploy: { restartPolicyType: "ON_FAILURE" },
    env: {
      ...agentEnv,
      DEFENSE_AUDIT: "/var/lib/defense/audit.jsonl",
  ...
  // The weekly league close. It exists because NOTHING closed the week: the
  // consumer is there (api/src/worker.ts:56 handles `league.week.close`) and the
  // calculation is there (api/src/leagues.ts closeWeek), but the only publisher
  // of that message in the whole repo is api/test/transport.mts. No scheduler
  // ever emitted it, so every user's league week stayed open forever.
  //
  // It runs the compiled script instead of publishing to the bus. The bus route
  // is the one worker.ts's comment describes, but this one has fewer moving
  // parts and still closes the week while RabbitMQ is down. The script is
  // idempotent by (user_id, week) with DO NOTHING, so a retry costs a query.
  //
  // 05:05 UTC Monday is 00:05 in America/Bogota — the zone is a constant in
  // api/src/leagues.ts:27, not an env var — and Railway cron expressions are
  // UTC. Five minutes past the cut-off so an attempt made at 23:59:59 counts.
  //
  // restartPolicyType NEVER: this process ends with `await pool.end()` and exits
  // 0. ON_FAILURE would be defensible; ALWAYS would restart a finished job into
  // a loop that closes the same week every few seconds.
  //
  // DATABASE_URL is the only variable it needs — verified by running the
  // compiled entrypoint with just that set and watching it reach ECONNREFUSED
  // rather than a missing-env throw. It gets NODE_ENV=production like every
  // other service here, which is also what makes api/scripts/league-demo.mjs
  // refuse to run in either environment.
  const cronLeagues = service("cron-leagues", {
    ...common,
    root: "app_ai_from_scratch",
    build: apiBuild,
    start: "node dist/api/scripts/close-leagues.mjs",
    deploy: { cronSchedule: "5 5 * * 1", restartPolicyType: "NEVER" },
    env: {
      DATABASE_URL: db.env.DATABASE_URL,
      NODE_ENV: "production",
      APP_ENV: prod ? "PROD" : "DEV",
