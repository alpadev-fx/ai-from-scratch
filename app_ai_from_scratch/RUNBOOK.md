...
| "turn on Sign in with Google" | Set `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` and `GOOGLE_REDIRECT_URI` on the **api** service, then restart it | The button does not exist until all three are set: `configGoogle` returns null, `GET /api/auth/google/estado` answers `{habilitado:false}` and the login and signup pages render no button. Partial configuration is treated as unconfigured on purpose — a wrong `redirect_uri` sends the authorization code somewhere else. In Google Cloud Console → Credentials → OAuth client (type: Web application), the **Authorized redirect URI** must match `GOOGLE_REDIRECT_URI` byte for byte: `https://aifromscratch.shop/api/v3/auth/google/callback` in production, `http://localhost:4321/api/v3/auth/google/callback` locally. |
